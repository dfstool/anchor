...
import GlobalModalAccountImportWatchDFS from './WatchDFS';
...
class GlobalModalAccountImportManual extends Component<Props> {
  ...
  onChange = (e, { name, valid, value }) => {
    ...
    switch (authType) {
      ...
      case 'key':
      default: {
        if (name === "account" || name === "permission" || name === "key") {
          valids[name] = valid;
          allValid = (
            valids.account
            && valids.permission
          ...
          const account = nameArray[0];
          const permission = nameArray[1];
          valids["account"] = valid; //checked
          valids["permission"] = valid; //checkd
          allValid = (
            valids.account
            && valids.permission
            && valids.key
