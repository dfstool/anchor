...
class ProducersDFS extends Component<Props> {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      ...
      if (settings.account && accounts[settings.account]) {
        ...
        const voters = this.props.dfs.voters;
        if (account.voter_info && voters[settings.account] && voters[settings.account].length) {
          const accountName = account.voter_info.proxy || account.account_name;
          let selected = editingProducers || !accounts[accountName] ?
            currentlySelected :
            [voters[settings.account][0].last_vote]
  ...
  addProducer = (producer) => {
    let producers = [...this.state.selected];
...
export default withTranslation('producers')(ProducersDFS);
