...
import ToolsFormCreateAccountConfirmingDFS from './CreateAccount/ConfirmingDFS';
...
class ToolsFormCreateAccountDFS extends Component<Props> {
  constructor(props) {
    super(props);
    const {
      accountName,
    ...
    if (connection.chain === 'DFS') {
      delegatedBw = '0.00000000';
      delegatedCpu = '0.00000000';
      // ramAmount = 2600;
      disableStakingFields = true;
      // disableRamField = true;
      disableDelegatedResourceWarning = true;
  ...
  errorsInForm = (errors, disabled) => {
    ...
    if (Number(ramAmount) < 2000) {
      formErrors.ramAmount = 'ram_under_minimum_of_2000_for_new_account';
      submitDisabled = true;
    }
  ...
  render() {
    ...
    return (
      ...
      >
              ...
              >
                ...
                {(ramPrice && !formErrors.ramAmount) ? (
                  <h4 style={{ margin: '30px' }}>
                    {`${t('tools_form_create_account_ram_price_estimate')} ${ramPrice.toFixed(connection.tokenPrecision)} ${connection.chainSymbol}.`}
                  </h4>
                ) : ''}
                <FormMessageError
        ...
        {(shouldShowConfirm)
          ? (
            <ToolsFormCreateAccountConfirmingDFS
...
export default withTranslation('tools')(ToolsFormCreateAccountDFS);
